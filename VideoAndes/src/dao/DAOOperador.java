...
public class DAOOperador {
	...
	public ArrayList<RentabilidadCompanias> generarReporte(String fechaIni, String fechaFin) throws SQLException{
		...
		String sql = "WITH B AS (SELECT * FROM SITIO si INNER JOIN TIPOSITIO ti ON si.ID =ti.ID  "
				...
				+ "NATURAL JOIN COMPANIA_ESPECTACULO "
				+ "GROUP BY  DIA, IDSITIO,TIPO, IDCOMPANIA, IDESPECTACULO, IDFUNCION, CAPACIDAD, CATEGORIA) "
				+ ", FIN AS (SELECT IDCOMPANIA, DIA, IDSITIO, TIPO, IDESPECTACULO, CATEGORIA, IDFUNCION, CANTFUNCION, CANTFUNCION/CAPACIDAD, VENDIDO "
				+ "FROM BONO1 WHERE DIA BETWEEN TO_DATE('"+fechaIni+"', 'mm/dd/yy') AND TO_DATE('"+fechaFin+"', 'mm/dd/yy') "
						+ "ORDER BY VENDIDO DESC) "
						+ "SELECT * FROM FIN LEFT JOIN (SELECT ID AS IDESPECTACULO, NOMBRE FROM ESPECTACULO) esp ON FIN.IDESPECTACULO=ESP.IDESPECTACULO";
