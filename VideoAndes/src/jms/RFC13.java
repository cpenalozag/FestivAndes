...
public class RFC13 implements MessageListener, ExceptionListener{

	public final static int TIME_OUT = 5;
	private final static String APP = "app1";

	private final static String GLOBAL_TOPIC_NAME = "java:global/RMQRFC13ALL";
	private final static String LOCAL_TOPIC_NAME = "java:global/RMQRFC13LOCAL";
	...
	ArrayList<FuncionBasica> answer = new ArrayList<>();

	public RFC13(TopicConnectionFactory factory, InitialContext ctx) throws JMSException, NamingException {
	...
	private void sendMessage(String payload, String status, Topic dest, String id) throws JMSException, JsonGenerationException, JsonMappingException, IOException
	{
		...
		ExchangeMsg msg = new ExchangeMsg("RFC13.app1", APP, payload, status, id);
	...
	public ArrayList<FuncionBasica> getRemoteRFC13(FiltrosConsultaFunciones filtros) throws NoSuchAlgorithmException, JsonGenerationException, JsonMappingException, IOException, JMSException, InterruptedException, NonReplyException{
		...
		ObjectMapper mapper = new ObjectMapper();
		String payload = mapper.writeValueAsString(filtros);
	...
	public void onMessage(Message message) {
		...
		try 
		{
			...
			if(!ex.getSender().equals(APP))
			{
				...
				{
					FiltrosConsultaFunciones filtros = mapper.readValue(ex.getPayload(), FiltrosConsultaFunciones.class);
					VideoAndesDistributed dtm = VideoAndesDistributed.getInstance();
					ArrayList<FuncionBasica> funciones = dtm.getLocalRFC13(filtros);
					String payload = mapper.writeValueAsString(funciones);
					Topic t = new RMQDestination("", "RFC13.test", ex.getRoutingKey(), "", false);
					sendMessage(payload, REQUEST_ANSWER, t, id);
				}
				//			7fdssdsfdsfd
				else if(ex.getStatus().equals(REQUEST_ANSWER))
				{
					ArrayList<FuncionBasica> reci = mapper.readValue(ex.getPayload(), ArrayList.class);
					answer.addAll(reci);
				}
			}
