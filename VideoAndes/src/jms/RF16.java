...
public class RF16 implements MessageListener, ExceptionListener {
	...
	private final static String GLOBAL_TOPIC_NAME = "java:global/RMQRF16ALL";
	private final static String LOCAL_TOPIC_NAME = "java:global/RMQRF16LOCAL";
	...
	private void sendMessage(String payload, String status, Topic dest, String id) throws JMSException, JsonGenerationException, JsonMappingException, IOException
	{
		...
		ExchangeMsg msg = new ExchangeMsg("RFC16.app1", APP, payload, status, id);
	...
	public ArrayList<NotaDebito> getRemoteRF16(String nombreCompania) throws NoSuchAlgorithmException, JsonGenerationException, JsonMappingException, IOException, JMSException, InterruptedException, NonReplyException{
		...
		ObjectMapper mapper = new ObjectMapper();
		String payload = mapper.writeValueAsString(nombreCompania);
	...
	public void onMessage(Message message) {
		...
		try 
		{
			...
			if(!ex.getSender().equals(APP))
			{
				...
				{
					String nombreCompania = mapper.readValue(ex.getPayload(), String.class);
					VideoAndesDistributed dtm = VideoAndesDistributed.getInstance();
					ArrayList<NotaDebito> recibos = dtm.getLocalRF16(nombreCompania);
					String payload = mapper.writeValueAsString(recibos);
					Topic t = new RMQDestination("", "RFC16.test", ex.getRoutingKey(), "", false);
					sendMessage(payload, REQUEST_ANSWER, t, id);
				}
//				7fdssdsfdsfd
				else if(ex.getStatus().equals(REQUEST_ANSWER))
				{
					ArrayList<NotaDebito> nd = mapper.readValue(ex.getPayload(), ArrayList.class);
					answer.addAll(nd);
				}
			}
